...
class DeepHistoneConfig:
    def __init__(self):
        ...
        self.FINAL_WINDOW_SIZE = 10000  # easy fix for larger context, just starting with 10 kb
...
def save_dataset_target_format(output_path, dna_onehot, dnase_scores, labels, genomic_keys, epigenome_id):
    ...
    #as per recommendation of durok (from Prof. Li's lab) concatenate the two features together 
    dna_dnase_concat = np.concatenate([dna_onehot, dnase_array], axis=2)
    
    np.savez_compressed(
        output_path,
        dna_dnase=dna_dnase_concat.astype(np.float64),  
...
def process_epigenome(epigenome_id, logger=None, output_dir=None, output_name=None):
    ...
    try:
        ...
        print(f"DEBUG: epigenome_id = {epigenome_id}")
        ...
        genomic_keys = np.array([f"{chrom}_{start}_{end}" for chrom, start, end in windows_1000bp], dtype='U50')
        ...
        save_dataset_target_format(output_path, dna_onehot, dnase_scores, labels, genomic_keys, epigenome_id)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DeepHistone preprocessing pipeline")
    parser.add_argument(
        "--epigenome", 
        type=str, 
        required=True, 
        help="Epigenome ID (e.g., E005)"
    )
    parser.add_argument(
        "--output_name", 
        type=str, 
        default=None, 
        help="Custom name for output file (default: <epigenome>_deephistone.npz)"
    )
    parser.add_argument(
        "--test_mode",
        action="store_true",
        help="Run in test mode (only process chr22)"
    )
    parser.add_argument(
    "--output_dir",
    type=str,
    ...
    # print(f"MAIN DEBUG: args.epigenome = {args.epigenome}")
    # print(f"MAIN DEBUG: args.output_dir = {args.output_dir}")
    # print(f"MAIN DEBUG: args.output_name = {args.output_name}")
    # print(f"MAIN DEBUG: args.test_mode = {args.test_mode}")
    ...
    # if args.output_name:
    #     def custom_output_path(epigenome_id):
    #         return os.path.join(config.OUTPUT_DIR, args.output_name)
    #     config.get_output_path = custom_output_path

    process_epigenome(
     epigenome_id=args.epigenome,
    logger=None,
