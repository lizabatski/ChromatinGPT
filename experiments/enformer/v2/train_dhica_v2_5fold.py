...
def train_single_model(train_index, valid_index, test_index,
                      model_config: dict,
                      training_config: dict,
                      data_file: str,
                      output_dir: str,
    ...
    # Create DataLoaders - IMPORTANT: Use num_workers=0 to avoid memory issues
    train_dataset = LazyHistoneDataset(data_file, train_index)
    valid_dataset = LazyHistoneDataset(data_file, valid_index)
    test_dataset = LazyHistoneDataset(data_file, test_index)
...
def main():
    ...
    # logger.info("Using single 60/20/20 train/validation/test split instead of 5-fold CV")
    ...
    # valid_end = int(n_samples * 0.8)

    # train_index = indexs[:train_end]
    # valid_index = indexs[train_end:valid_end] 
    # test_index = indexs[valid_end:]

    # logger.info(f"Split sizes:")
    # logger.info(f"  Train: {len(train_index)} samples ({len(train_index)/n_samples*100:.1f}%)")
    # logger.info(f"  Valid: {len(valid_index)} samples ({len(valid_index)/n_samples*100:.1f}%)")
    # logger.info(f"  Test: {len(test_index)} samples ({len(test_index)/n_samples*100:.1f}%)")
    ...
    #     for histone in histones:
    #         if histone in test_auPRC:
    #             print(f"  {histone}: auPRC={test_auPRC[histone]:.4f}, auROC={test_auROC[histone]:.4f}")
